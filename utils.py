...
def load_graph_data(dataset_name, show_details=False):
    ...
    :return: the features, labels and adj, node_num, cluster number
    """
...
def add_gaussian_noise(X, sigma):
    ...
    noise = torch.normal(mean=0, std=sigma, size=X.size())
    return X + noise

def apply_random_mask(X, mask_ratio):
    """Apply a random mask to the input."""
    mask = torch.rand(X.shape) > mask_ratio
    return X * mask.float()

def Hybrid_Collaborative_Augmentation(A, Xn, Xm, t_l, t_m):
    A_tmp = A - torch.diag_embed(torch.diag(A))
    A_norm = normalize_adj(A_tmp, self_loop=True, symmetry=True)
    if args.n_input != -1 or args.dataset == 'amap':
        I = torch.eye(A.shape[0]).float()
        L = I - A_norm.float()
    else:
        I = torch.eye(A.shape[0])
        L = I - A_norm
    Xn = torch.tensor(Xn) if not isinstance(Xn, torch.Tensor) else Xn
    Xm = torch.tensor(Xm) if not isinstance(Xm, torch.Tensor) else Xm

    X_l = add_gaussian_noise(Xn, sigma=args.sigma)
    X_m = apply_random_mask(Xm, mask_ratio=args.mask_ratio)
...
def comprehensive_similarity(Za, Zb, Ea, Eb, alpha):
    Za_Zb = torch.cat([torch.cat([Za @ Za.T, Za @ Zb.T], dim=1),
                       torch.cat([Zb @ Za.T, Zb @ Zb.T], dim=1)], dim=0)

    Ea_Eb = torch.cat([torch.cat([Ea @ Ea.T, Ea @ Eb.T], dim=1),
                       torch.cat([Eb @ Ea.T, Eb @ Eb.T], dim=1)], dim=0)

    S = alpha * Za_Zb + (1 - alpha) * Ea_Eb
    return S


def Sample_Adaptive_Differential_Awareness_infoNCE(S, W, pos_neg_weight, pos_weight, node_num):
    pos_neg = W * torch.exp(S * pos_neg_weight)
    ...
    loss = (-torch.log(pos / (pos + neg))).sum() / (2 * node_num)
    return loss
...
def high_confidence(Z, center):
    ...
    value, _ = torch.topk(distance_norm, int(Z.shape[0] * (1 - args.tao_l)))
...
def pseudo_matrix(P, S, node_num):
    ...
    W_1 = torch.exp(torch.abs(Q - S_norm) ** args.beta * Q)
    W_0 = torch.exp(torch.abs(1 - S_norm) ** args.gama * (1 - Q))
    W_mat = W_1 + W_0
    W = torch.cat([torch.diag(W_mat, node_num), torch.diag(W_mat, -node_num)], dim=0)
    return W, W_mat
